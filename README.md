...
```sh
make docker-build docker-push IMG=<some-registry>/headless-istio-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/headless-istio-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/headless-istio-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/headless-istio-operator/<tag or branch>/dist/install.yaml
```
